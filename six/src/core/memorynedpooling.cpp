...
namespace six {
  ...
  //---------------------------------------------------------------------
  void* NedPooling::allocBytes(size_t count, const char* file, int line, const char* func) {
    void* ptr = six_nedpooling::internalAlloc(count);
#if MEMORY_TRACKER
  ...
  //---------------------------------------------------------------------
  void NedPooling::deallocBytes(void* ptr) {
    // deal with null
    if (!ptr)
      return;
#if MEMORY_TRACKER
...
#endif
    six_nedpooling::internalFree(ptr);
  }
  //---------------------------------------------------------------------
  void* NedPooling::allocBytesAligned(size_t align, size_t count, const char* file, int line, const char* func) {
    // default to platform SIMD alignment if none specified
    void* ptr =  align ? six_nedpooling::internalAllocAligned(align, count) : six_nedpooling::internalAllocAligned(16, count);
#if MEMORY_TRACKER
    MemoryTracker::get()._recordAlloc(ptr, count, 0, file, line, func);
#else
    // avoid unused params warning
  ...
  //---------------------------------------------------------------------
  void NedPooling::deallocBytesAligned(size_t align, void* ptr) {
    // deal with null
    if (!ptr)
      return;
#if MEMORY_TRACKER
