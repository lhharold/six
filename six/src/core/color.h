...
namespace six {
	...
	public:
		...
		const Color4i& operator += (const Color4i& clr) {return (*this = *this+clr);}
		...
		const Color4i& operator *= (f32 val) {return (*this = *this*val);}
	};
	class Color4f {
	public:
    static const Color4f Zero;
		...
		const Color4f& operator += (const Color4f& clr) {return (*this = *this+clr);}
		Color4f operator - (const Color4f& clr) const{return Color4f(r-clr.r, g-clr.g, b-clr.b, a-clr.a);}
		const Color4f& operator -= (const Color4f& clr) {return (*this = *this-clr);}
		Color4f operator * (f32 val)const {return Color4f(r*val, g*val, b*val, a*val);}
		const Color4f& operator *= (f32 val) {return (*this = *this*val);}
		Color4f operator * (const Color4f& clr)const {return Color4f(r*clr.r, g*clr.g, b*clr.b, a*clr.a);}
		const Color4f& operator *= (const Color4f& clr) {return (*this = *this*clr);}
	};
