...
namespace six {
  ...
  public:
    ...
    Color4i operator - (const Color4i& clr) const{u8 minV = 0; return Color4i(MAX(mimV, r-clr.r), MAX(minV, g-clr.g), MIN(minV, b-clr.b), MIN(minV, a-clr.a));}
    Color4i& operator -= (const Color4i& clr) {return (*this = *this-clr);}
		Color4i operator * (f32 val)const {return Color4i((u8)(CLAMP(FROUND(r*val), 0, 255)), (u8)(CLAMP(FROUND(g*val), 0, 255)), (u8)(CLAMP(FROUND(b*val), 0, 255)), (u8)(CLAMP(FROUND(a*val), 0, 255)));}
  ...
  class Color4f {
    ...
    bool operator == (const Color4f& clr) const {return FEQUAL(r, clr.r) && FEQUAL(g, clr.g) && FEQUAL(b, clr.b) && FEQUAL(a, clr.a);}
